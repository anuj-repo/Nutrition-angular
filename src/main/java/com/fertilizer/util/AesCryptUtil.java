...
public class AesCryptUtil {
    ...
    */
    public String decrypt(String hexCipherText){
        try{
            String plaintext = new String(dcipher.doFinal(AesCryptUtil.hexToByte(hexCipherText)), "UTF-8");
            return  plaintext;
        } catch (Exception e){
            e.printStackTrace();
            return null;
    ...
    public static String encodeData(String content, String secretKey) {
        try {
            return Base64.getEncoder().encodeToString((secretKey+content).getBytes("utf-8"));
        } catch (Exception ex) {
        	System.out.println("error : Exception in error : " + ex);
        }
		return null;
    }
